...
import {roundedBox,tube,leafCard,rockMesh,coniferMesh,strataRockMesh} from '../dist/js/meshes.js';
...
for(const make of [makeCube,makeCylinder,makeSphere,roundedBox,tube,leafCard,rockMesh,coniferMesh,strataRockMesh]){const v=make();assert(v.every(Number.isFinite));assert.equal(v.length%24,0);for(let i=0;i<v.length;i+=24){const a=[v[i+8]-v[i],v[i+9]-v[i+1],v[i+10]-v[i+2]],b=[v[i+16]-v[i],v[i+17]-v[i+1],v[i+18]-v[i+2]],n=[a[1]*b[2]-a[2]*b[1],a[2]*b[0]-a[0]*b[2],a[0]*b[1]-a[1]*b[0]];assert(n[0]*v[i+3]+n[1]*v[i+4]+n[2]*v[i+5]>=-.00001,'Inward triangle winding');}}
