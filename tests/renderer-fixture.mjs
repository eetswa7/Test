...
import {roundedBox,tube,leafCard,rockMesh,groundSurface,coniferMesh,coniferTint,strataRockMesh,strataTint} from '../dist/js/meshes.js';
...
export function fixture(){
 ...
 r.lightingField=new LightingField();r.roomLights=new RoomLights();
 ...
 for(const [key,make]of Object.entries({cube:makeCube,surface:groundSurface,cylinder:()=>makeCylinder(16),sphere:makeSphere,bevel:()=>roundedBox(.1,4),bevelWorld:()=>roundedBox(.08,3),tube,leaf:leafCard,rock:rockMesh,conifer:coniferMesh,strata:strataRockMesh,bevelActor:()=>roundedBox(.1,2)})){
 ...
 const stone=r.geometry.strata.getAttribute('position'),stoneColors=new Float32Array(stone.count*3);
 for(let i=0;i<stone.count;i++){color.setRGB(...strataTint(stone.getY(i)),THREE.SRGBColorSpace);stoneColors.set(color.toArray(),i*3);}
 r.geometry.strata.setAttribute('color',new THREE.BufferAttribute(stoneColors,3));
 return r;
}
