...
const kinds=['RIFLE','RIFLE','RIFLE','SMG','SMG','SHOTGUN','SHOTGUN','SNIPER','MARKSMAN','LMG','PISTOL','PISTOL','MELEE','RIFLE','SMG','MARKSMAN'];
...
test('every arsenal entry builds a distinct mobile-sized view model with finite transforms',()=>{
 ...
 for(let id=0;id<16;id++)for(const optic of [0,1,2,3]){
...
test('all attachment combinations animate cached arrays without drifting or replacing parts',()=>{
 for(let id=0;id<16;id++)for(const grip of [0,1,2,4]){
...
test('muzzles follow real barrel tips and suppressors extend the origin',()=>{
 for(let id=0;id<16;id++){if(id===12)continue;const a=muzzlePosition(weapon(id)),s=muzzlePosition(weapon(id,1,1));assert(s.z<a.z-.1);assert.equal(a.x,0);assert(a.z<-.15);assert(a.y>0);}
});
test('reflex and prism optics leave a clear central sight ray',()=>{
 for(let id=0;id<16;id++)for(const optic of [1,2]){
  if(id===7||id===12)continue;const p=weaponModel(weapon(id,optic));
