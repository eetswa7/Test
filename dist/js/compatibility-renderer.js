import {identity,lookAt,multiply,compose,direction,clamp,lerp,distance} from './math.js?v=6';
import {weaponModel,actorModel,part,material,makeCube,makeCylinder,makeSphere} from './geometry.js?v=6';
...
import {loadImages} from './textures.js?v=6';
...
export class CompatibilityRenderer {
 ...
 render(game,dt,menu,elapsed=dt){
  ...
  if(!p.dead&&(menu||!scopeVisible(p))){
   const weapon=p.weapon,key=[weapon.def.id,weapon.optic,weapon.barrel,weapon.grip].join('/');if(key!==this.weaponKey){this.weaponKey=key;this.weaponParts=weaponModel(weapon);}
