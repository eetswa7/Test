import {animateWeaponParts} from './weapon-models.js?v=40';
import {identityFor} from './combat-identity.js?v=40';
import {identity,lookAt,multiply,compose,direction,clamp,lerp,distance} from './math.js?v=40';
import {weaponModel,actorModel,part,material,makeCube,makeCylinder,makeSphere} from './geometry.js?v=40';
import {roundedBox,tube,leafCard,rockMesh,ridgeMesh,ridgeTint,coniferMesh,coniferTint,strataRockMesh,strataTint} from './meshes.js?v=40';
import {aimFov,verticalFov,scopeVisible,weaponPose} from './aim.js?v=40';
import {loadImages} from './textures.js?v=40';
import {weatherParticles} from './particles.js?v=40';
...
export class CompatibilityRenderer {
 ...
 render(game,dt,menu,elapsed=dt){
  ...
  for(const g of game.grenades)polygons.push(...this.box(part(g.x,g.y,g.z,.16,.16,.16,'green')));
  for(const w of weatherParticles(this,game.time??0))polygons.push(...this.box(part(w.x,w.y,w.z,w.sizeX,w.sizeY,w.sizeZ,w.kind?'white':'stone',{yaw:w.yaw,color:w.color})));
  this.paint(polygons);
