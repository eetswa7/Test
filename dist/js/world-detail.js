...
export function dressWorld(arena){
 ...
 if(id!==1)for(const [x,z]of treeSpots)if(free(x,z,.65)){
  arena.box(x,.8,z,.42,1.6,.42,'bark',{invisible:true});
  if(id===3)tree(x,z,5+random()*2);else palm(x,z,5.5+random()*2.5);
  plant(x+.8,z,.65,3);
 }
 // Continuous surrounding hills and foliage replace the old rectangular skyline.
 for(let i=0;i<36;i++){
  const angle=i/36*Math.PI*2,radius=s+17+random()*16,x=Math.sin(angle)*radius,z=Math.cos(angle)*radius;
  ...
  if(id!==1&&i%2===0){if(id===3)tree(x,z,7+random()*4);else palm(x,z,7+random()*3);}
  if(i%2)plant(x,z,3+random()*2,0);
 }
 // Light rubble, low planters and sandbags are chamfered to catch the sunlight.
 for(const b of arena.blocks){if(b.h>.7&&b.h<1.5&&!b.ground&&b.w>2){b.mesh='bevel';for(let i=0;i<Math.floor(b.w);i++)add(b.x-b.w/2+.6+i,b.y+b.h/2+.10,b.z,.82,.23,.54,'fabric',{mesh:'bevel',color:[.88,.8,.60]});}}
}
