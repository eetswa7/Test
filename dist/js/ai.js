import {distance,direction,angleDelta,clamp} from './math.js?v=11';
...
export function updateBot(bot,dt,e){
 ...
 if(bot.aiClock<=0){
  ...
  if(bot.goal&&bot.pathClock<=0){bot.path=e.nav.path(bot,bot.goal);bot.pathIndex=0;bot.pathClock=.85+e.random()*.55;}
 }
 const target=bot.target===null?null:e.actors.find(a=>a.id===bot.target&&!a.dead);
 let moving=false;
 if(bot.pathIndex<bot.path.length){let p=bot.path[bot.pathIndex],dx=p.x-bot.x,dz=p.z-bot.z,n=Math.hypot(dx,dz);if(n<.42)bot.pathIndex++;else {let speed=(bot.state==='evade'?5.8:bot.state==='engage'?1.8:bot.role===1?4.5:3.4)*e.difficulty.speed;e.moveActor(bot,dx/n*speed,dz/n*speed,dt);moving=true;if(!target)bot.yaw+=angleDelta(bot.yaw,Math.atan2(dx,-dz))*clamp(dt*8,0,1);}}
