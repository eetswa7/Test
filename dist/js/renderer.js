...
export class Renderer {
 ...
 render(game,dt,menu=false){
  ...
  lookAt(this.shadowView,{x:cx+sun[0]*65,y:55,z:cz+sun[2]*65},{x:cx,y:0,z:cz});ortho(this.shadowProj,42,1,140);multiply(this.shadowVP,this.shadowProj,this.shadowView);
  this.shadowClock+=dt;if(this.hasShadow&&levels[this.quality].shadow&&this.shadowClock>=1/30){this.shadowClock=0;gl.bindFramebuffer(gl.FRAMEBUFFER,this.shadowFBO);gl.viewport(0,0,1024,1024);gl.clear(gl.DEPTH_BUFFER_BIT);gl.useProgram(this.shadow);gl.uniformMatrix4fv(this.uniform(this.shadow,'uVP'),false,this.shadowVP);gl.enable(gl.POLYGON_OFFSET_FILL);gl.polygonOffset(1.5,2);this.draw('world');this.draw('actors');gl.disable(gl.POLYGON_OFFSET_FILL);}
