import {clamp,lerp} from './math.js?v=16';
...
export function weaponPose(p,time,motion=true,menu=false,out={}){
 ...
 out.x=lerp(short?.18:.22,0,ads)+sway+reload*(shell?.018:.035);
 out.y=lerp(short?-.218:-.245,-sightHeight(w),ads)+bob-reload*.11*reloadTilt-(p.switchLeft??0)*.42-sprint*.075;
 ...
 out.roll=lerp(short?-.02:-.035,0,ads)-reload*.22*reloadTilt+sprint*.18+(motion?sign*kick*.11*hip:0);
