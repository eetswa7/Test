...
import {clamp} from './math.js?v=16';
...
export function bakeLightField(arena,resolution=64){
  const size=arena.info.size,data=new Uint8Array(resolution*resolution*4);
...
export class LightingField {
  ...
  setArena(arena){
    const field=bakeLightField(arena);this.texture.value?.dispose();
  ...
  dispose(){this.texture.value?.dispose();}
}
